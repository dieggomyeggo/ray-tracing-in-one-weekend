...
fn ray_color(r: &ray::Ray) -> nalgebra::Vector3<f64> {
    if hit_sphere(nalgebra::Vector3::new(0.0, 0.0, -1.0), 0.5, r) {
...
static VIEWPORT_W: f64 = VIEWPORT_H * ((IMG_W as f64 / IMG_H as f64) as f64);
...
fn hit_sphere(center: Vector3<f64>, radius: f64, r: &ray::Ray) -> bool {
    let oc = center - r.origin();
    let a = nalgebra::Vector3::dot(r.direction(), r.direction());
    let b = -2.0 * nalgebra::Vector3::dot(r.direction(), &oc);
    let c = nalgebra::Vector3::dot(&oc, &oc) - (radius * radius);
